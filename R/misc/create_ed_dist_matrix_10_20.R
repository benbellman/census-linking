...
ed1_poly <- st_read(here("data", "shertzer_eds", "Philadelphia_1910.shp")) %>% 
  st_transform(26918) %>% 
  mutate(ed1 = as.character(ED)) %>% 
  select(-ED)

ed2_poly <- st_read(here("data", "shertzer_eds", "Philadelphia_1920.shp")) %>% 
  st_transform(26918) %>% 
...
calc_ed_dist <- function(ed1_id, ed2_id, ed1, ed2){
  st_distance(st_centroid(filter(ed1, ed1 == ed1_id)), st_centroid(filter(ed2, dest_ed == ed2_id)))[[1]]
}

#dist_test <- slice(dist_matrix, 1:600)

plan(tweak(multiprocess, workers = 32))

# compute all distances for distance matrix for each ED combination between years
dist_matrix$dist <- suppressWarnings(future_map2_dbl(dist_matrix$ed1, dist_matrix$dest_ed, calc_ed_dist, ed1_poly, ed2_poly))

# export distance matrix
write_csv(dist_matrix, here("data", "ed_distances_10_20.csv"))
