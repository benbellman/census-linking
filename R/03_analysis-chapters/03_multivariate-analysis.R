...
linked <- import(here("data", "analysis", "links-full-info-2.csv")) %>% 
  ...
  mutate(
    moved = if_else(move_type == "Long Move", 1, 0),
    woman = sex1 - 1,
...
coefs %>% 
  ...
  ggplot() +
  geom_point(aes(x = spec_type, y = or, col = spec_type)) +
  facet_grid(race_cat ~ .) +
  coord_flip()
...
write_csv(black_coefs, here("tables", "new_chp3", "black_logit_coefs2.csv"))
...
write_csv(wimm_coefs, here("tables", "new_chp3", "wimm_logit_coefs2.csv"))
...
write_csv(wnb_coefs, here("tables", "new_chp3", "wnb_logit_coefs2.csv"))
...
# export a table of mean values for source variables
select(linked, race_cat, year1, model_vars) %>% 
  ...
  write_csv(here("tables", "new_chp3", "logit_var_means2.csv"))
...
# compare specs using NB White in 1930-40 (White NB)
ex_predictions %>% 
  filter(race_cat == "White NB" & year1 == 1930) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  ...
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_wnb_1930.png"), width = 9, height = 11)

# compare specs using NB White in 1920-30 (White NB)
ex_predictions %>% 
  filter(race_cat == "White NB" & year1 == 1920) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_wnb_1920.png"), width = 9, height = 11)

# compare specs using NB White in 1910-20 (White NB)
ex_predictions %>% 
  filter(race_cat == "White NB" & year1 == 1910) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
...
# compare specs using NB White in 1930-40 (White Imm)
ex_predictions %>% 
  filter(race_cat == "White Imm" & year1 == 1930) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_wimm_1930.png"), width = 9, height = 11)

# compare specs using NB White in 1920-30 (White Imm)
ex_predictions %>% 
  filter(race_cat == "White Imm" & year1 == 1920) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_wimm_1920.png"), width = 9, height = 11)

# compare specs using NB White in 1910-20 (White Imm)
ex_predictions %>% 
  filter(race_cat == "White Imm" & year1 == 1910) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_wimm_1910.png"), width = 9, height = 11)


# compare specs using NB White in 1930-40 (Black)
ex_predictions %>% 
  filter(race_cat == "Black" & year1 == 1930) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_black_1930.png"), width = 9, height = 11)

# compare specs using NB White in 1920-30 (Black)
ex_predictions %>% 
  filter(race_cat == "Black" & year1 == 1920) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_black_1920.png"), width = 9, height = 11)

# compare specs using NB White in 1910-20 (Black)
ex_predictions %>% 
  filter(race_cat == "Black" & year1 == 1910) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(spec_type ~ ex) +
  ggsave(here("figures", "new_chp3", "map_testing_owner_interactions_black_1910.png"), width = 9, height = 11)


### deviations between predicted and observed move rates

# plot dev of mean predicted probabilities from observed rates(White NB)
obs_predictions %>% 
  filter(race_cat == "White NB" & spec_type %in% c("Linear, none", "Linear, ownership")) %>% 
  ggplot() +
  geom_sf(data = rbind(phl10, phl20, phl30), fill = "grey", lwd = 0, col = "white") +
  geom_sf(aes(geometry = geometry, fill = dev_map), lwd = 0, col = "white") +
  scale_fill_manual(values = c(brewer.pal(7, "RdYlBu"), "grey")) +
  coord_sf(xlim = c(1740000, 1756000), ylim = c(458000, 480000)) +
  theme_bw() +
...
#### Predictions of hypothetical HH moving given real EDs, no interactions

# homeowner in time 1
ex_predictions %>% 
  filter(spec_type == "Linear, none" & homeowner == 1) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(year1 ~ race_cat) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "owner_prediction_maps_noint.png"), width = 9, height = 9)

ex_predictions %>% 
  filter(spec_type == "Linear, ownership" & homeowner == 1) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(year1 ~ race_cat) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "owner_prediction_maps_yesint.png"), width = 9, height = 9)



# renter in time 1
ex_predictions %>% 
  filter(spec_type == "Linear, none" & homeowner == 0) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(year1 ~ race_cat) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "renter_prediction_maps_noint.png"), width = 9, height = 9)

ex_predictions %>% 
  filter(spec_type == "Linear, ownership" & homeowner == 0) %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prob_map), lwd = 0, col = "white") +
  scale_fill_brewer("Predicted\nProbability", 
                    palette = "BuPu", 
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_grid(year1 ~ race_cat) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "renter_prediction_maps_yesint.png"), width = 9, height = 9)
...
ex_ed_pblack <- all_row_combos(rbind(ex_own, ex_rent), ex_ed_pblack)
ex_ed_pfrnbrn <- all_row_combos(rbind(ex_own, ex_rent), ex_ed_pfrnbrn)
ex_ed_msei <- all_row_combos(rbind(ex_own, ex_rent), ex_ed_msei)

# combine with 9 models for groups and years
all_data_models %>% 
  filter(spec_type %in% c("Linear, none", "Linear, ownership")) %>% 
  select(race_cat, year1, model, spec_type) %>% 
  mutate(ex_data = rep(list(ex_ed_pblack), 18),
         pred = map2(model, ex_data, my_predict)) %>% 
  select(race_cat, year1, ex_data, pred, spec_type) %>% 
  unnest(cols = c(ex_data, pred)) -> ed_pblack_preds

all_data_models %>% 
  filter(spec_type %in% c("Linear, none", "Linear, ownership")) %>% 
  select(race_cat, year1, model, spec_type) %>% 
  mutate(ex_data = rep(list(ex_ed_pfrnbrn), 18),
         pred = map2(model, ex_data, my_predict)) %>% 
  select(race_cat, year1, ex_data, pred, spec_type) %>% 
...
all_data_models %>% 
  filter(spec_type %in% c("Linear, none", "Linear, ownership")) %>% 
  select(race_cat, year1, model, spec_type) %>% 
  mutate(ex_data = rep(list(ex_ed_msei), 18),
         pred = map2(model, ex_data, my_predict)) %>% 
  select(race_cat, year1, ex_data, pred, spec_type) %>% 
...
# % black
filter(ed_pblack_preds, spec_type == "Linear, none") %>% 
ggplot() +
  geom_hline(yintercept = 0.5, col = "red") +
  geom_line(aes(x = ed_pct_black1, y = prob, col = race_cat, lty = ex), size = 0.8) +
  #scale_color_manual(values = ghibli_palette("PonyoMedium")[c(1,3,4)]) +
  scale_color_manual(values = c("#ABABAB","#6F6F6F","#101010")) +
  ...
  #xlim(0, 50) +
  ggsave(here("figures", "new_chp3", "final figures", "pblack_predictions_lines_noint.png"), height = 3, width = 6)

filter(ed_pfrnbrn_preds, spec_type == "Linear, none") %>% 
ggplot() +
  geom_hline(yintercept = 0.5, col = "red") +
  geom_line(aes(x = ed_pct_frnbrn1, y = prob, col = race_cat, lty = ex), size = 0.8) +
  scale_color_manual(values = c("#ABABAB","#6F6F6F","#101010")) +
  labs(x = "ED % immigrant",
       y = "Probability of moving",
       col = "",
  ...
  theme(panel.grid = element_blank()) +
  #xlim(0, 65) +
  ggsave(here("figures", "new_chp3", "final figures", "pfrnbrn_predictions_lines_noint.png"), height = 3, width = 6)

filter(ed_msei_preds, spec_type == "Linear, none") %>% 
ggplot() +
  geom_hline(yintercept = 0.5, col = "red") +
  geom_line(aes(x = ed_mean_sei1, y = prob, col = race_cat, lty = ex), size = 0.8) +
  scale_color_manual(values = c("#ABABAB","#6F6F6F","#101010")) +
  labs(x = "ED mean SEI",
       y = "Probability of moving",
       col = "",
  ...
  theme(panel.grid = element_blank()) +
  xlim(5, 30) +
  ggsave(here("figures", "new_chp3", "final figures", "msei_predictions_lines_noint.png"), height = 3, width = 6)
...
####
# let's directly compare with/without interactions within each group
# black households
filter(ed_msei_preds, race_cat == "Black") %>% 
  ggplot() +
  geom_line(aes(x = ed_mean_sei1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED mean SEI",
       y = "Probability of moving",
       col = "",
       lty = "") +
  ...
  xlim(5, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "black_msei_compare.png"), height = 3, width = 6)

filter(ed_pblack_preds, race_cat == "Black") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_black1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Black",
       y = "Probability of moving",
       col = "",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "black_pblack_compare.png"), height = 3, width = 6)

filter(ed_pfrnbrn_preds, race_cat == "Black") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_frnbrn1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Immigrant",
       y = "Probability of moving",
       col = "",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "black_pfrnbrn_compare.png"), height = 3, width = 6)


# immigrant households
filter(ed_msei_preds, race_cat == "White Imm") %>% 
  ggplot() +
  geom_line(aes(x = ed_mean_sei1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED mean SEI",
  ...
  theme(panel.grid.minor = element_blank()) +
  xlim(5, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wimm_msei_compare.png"), height = 3, width = 6)

filter(ed_pblack_preds, race_cat == "White Imm") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_black1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Black",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wimm_pblack_compare.png"), height = 3, width = 6)

filter(ed_pfrnbrn_preds, race_cat == "White Imm") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_frnbrn1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Immigrant",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wimm_pfrnbrn_compare.png"), height = 3, width = 6)


# native-born white households
filter(ed_msei_preds, race_cat == "White NB") %>% 
  ggplot() +
  geom_line(aes(x = ed_mean_sei1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED mean SEI",
  ...
  theme(panel.grid.minor = element_blank()) +
  xlim(5, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wnb_msei_compare.png"), height = 3, width = 6)

filter(ed_pblack_preds, race_cat == "White NB") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_black1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Black",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wnb_pblack_compare.png"), height = 3, width = 6)

filter(ed_pfrnbrn_preds, race_cat == "White NB") %>% 
  ggplot() +
  geom_line(aes(x = ed_pct_frnbrn1, y = prob, col = spec_type, lty = ex)) +
  scale_color_manual(values = c("#CCCCCC","#808080","#101010")) +
  labs(x = "ED % Immigrant",
  ...
  theme(panel.grid.minor = element_blank()) +
  #xlim(0, 30) +
  ggsave(here("figures", "new_chp3", "interaction_viz", "wnb_pfrnbrn_compare.png"), height = 3, width = 6)
